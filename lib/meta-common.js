...
async function instagramCandidates(userAccessToken) {
  ...
  return Promise.all((pages.data || []).filter(item => item.instagram_business_account?.id && item.access_token).map(async item => {
...
module.exports = {
  ...
  tokenCookieName, pendingCookieName, pack, unpack, parseCookies, cookie, redirect, panelUrl, graphJson, instagramCandidates,
};
